//! Thin shim that re-exports `my_fam_tree_api::ApiDoc` so the `openapi-dump`
//! binary and external consumers (CI, FE codegen) keep the
//! `my_fam_tree_openapi::ApiDoc` path stable.
//!
...
//!
//! [`build_app`]: my_fam_tree_api::build_app

pub use my_fam_tree_api::ApiDoc;
