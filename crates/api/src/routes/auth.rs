...
/// Cheap, deliberately permissive email syntax check.
///
/// Full RFC 5322 validation belongs at the email-sender layer; here we only
/// reject obvious nonsense so the rate-limiter key isn't polluted by typos.
...
mod tests {
    ...
    #[test]
    fn looks_like_email_accepts_at_sign_only() {
        assert!(looks_like_email("a@b.c"));
        assert!(looks_like_email("user+tag@example.org"));
