// Backend renders email links from `WEB_PUBLIC_URL` (= `http://my-family.docker`).
...
const E2E_BASE_URL = process.env['E2E_BASE_URL'] ?? 'http://my-family.docker'

export function rewriteEmailLink(link: string): string {
    return link.replace(/^https?:\/\/my-family\.docker(?::\d+)?/, E2E_BASE_URL)
}
